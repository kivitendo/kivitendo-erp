...
UPDATE ar SET record_type = 'ar_transaction_storno'
  WHERE record_type IS NULL and (invoice = FALSE or invoice IS NULL) and storno_id IS NOT NULL;
...
UPDATE ar SET record_type = 'invoice_for_advance_payment_storno'
  WHERE record_type IS NULL and type = 'invoice_for_advance_payment' and amount < 0 and storno_id IS NOT NULL;
...
UPDATE ar SET record_type = 'credit_note_storno'
  WHERE record_type IS NULL and type = 'credit_note' and amount > 0 and storno_id IS NOT NULL;
...
UPDATE ap SET record_type = 'ap_transaction_storno'
  WHERE record_type IS NULL and (invoice = FALSE or invoice IS NULL) and storno_id IS NULL;
...
ALTER TABLE ap ALTER COLUMN record_type SET NOT NULL;
