...
sub action_set_zugferd_option {
  ...
  # Save to db. Delete entry if not active and all field are empty.
  my $zugferd_option = SL::DB::Manager::FileVersionsZugferdOption->find_by_or_create(guid => $::form->{zugferd_option}{guid});

  if ($zugferd_option->id && !($::form->{zugferd_option}{attach} || $::form->{zugferd_option}{doc_id} || $::form->{zugferd_option}{doc_name})) {
    $zugferd_option->delete;
  } else {
