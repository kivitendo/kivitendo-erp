...
sub all_transactions {
  ...
  my ($unbalanced_accounts, @unbalanced_accounts_value);
  if ($form->{show_unbalanced}) {
    $unbalanced_accounts = qq| AND ( a.paid <> a.amount OR a.datepaid >= ?) |;
    @unbalanced_accounts_value = (conv_date($form->{todate}));
  }
  ...
  foreach my $id (@id) {
    ...
    $query =
      qq|SELECT ac.itime, a.id, a.reference, a.description, '' as number, ac.transdate, ac.chart_id, | .
      ...
      qq|SELECT ac.itime, a.id, a.invnumber, c.name, c.customernumber as number, ac.transdate, ac.chart_id, | .
      ...
      qq|SELECT ac.itime, a.id, a.invnumber, v.name, v.vendornumber as number, ac.transdate, ac.chart_id, | .
    ...
    push(@values,
         ...
         @where_values, @department_values, @project_values, @unbalanced_accounts_value, $id,
         @where_values, @department_values, @project_values, @unbalanced_accounts_value, $id);
